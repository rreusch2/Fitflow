...
extension AIService {
    ...
    func parseWorkoutPlanResponse(_ response: String, userId: UUID) throws -> WorkoutPlan {
        guard let data = response.data(using: .utf8) else {
            throw AIError.parsingError("Invalid response encoding")
        }
        
        do {
            let jsonResponse = try JSONDecoder().decode(WorkoutPlanResponse.self, from: data)
            ...
            return WorkoutPlan(
                ...
                difficultyLevel: .intermediate, // Default, could be parsed from response
                estimatedDuration: jsonResponse.estimatedDuration,
                targetMuscleGroups: jsonResponse.targetMuscleGroups.compactMap { MuscleGroup(rawValue: $0) },
                equipment: [], // Would be parsed from exercises
                exercises: jsonResponse.exercises.map { exerciseResponse in
                    Exercise(
                        id: UUID(),
                        name: exerciseResponse.name,
                        description: exerciseResponse.description,
                        muscleGroups: exerciseResponse.muscleGroups.compactMap { MuscleGroup(rawValue: $0) },
                        equipment: Equipment(rawValue: exerciseResponse.equipment ?? "none"),
                        sets: exerciseResponse.sets,
                        reps: exerciseResponse.reps,
                        weight: exerciseResponse.weight,
                        restTime: exerciseResponse.restTime,
                        instructions: exerciseResponse.instructions,
                        tips: exerciseResponse.tips,
                        modifications: exerciseResponse.modifications.map { modResponse in
                            ExerciseModification(
                                type: ModificationType(rawValue: modResponse.type) ?? .easier,
                                description: modResponse.description,
                                instructions: modResponse.instructions
                            )
                        },
                        videoUrl: nil,
                        imageUrl: nil
                ...
                aiGeneratedNotes: jsonResponse.aiGeneratedNotes,
                createdAt: Date(),
                updatedAt: Date()
            )
    ...
    func parseMealPlanResponse(_ response: String, userId: UUID) throws -> MealPlan {
        guard let data = response.data(using: .utf8) else {
            throw AIError.parsingError("Invalid response encoding")
        }
        
        do {
            let jsonResponse = try JSONDecoder().decode(MealPlanResponse.self, from: data)
            ...
            return MealPlan(
                ...
                title: jsonResponse.title,
                description: jsonResponse.description,
                targetCalories: jsonResponse.targetCalories,
                macroBreakdown: MacroBreakdown(
                    protein: jsonResponse.macroBreakdown.protein,
                    carbs: jsonResponse.macroBreakdown.carbs,
                    fat: jsonResponse.macroBreakdown.fat,
                    fiber: jsonResponse.macroBreakdown.fiber
                ),
                meals: jsonResponse.meals.map { mealResponse in
                    Meal(
                        id: UUID(),
                        type: MealType(rawValue: mealResponse.type) ?? .breakfast,
                        name: mealResponse.name,
                        description: mealResponse.description,
                        calories: mealResponse.calories,
                        macros: MacroBreakdown(
                            protein: mealResponse.macros.protein,
                            carbs: mealResponse.macros.carbs,
                            fat: mealResponse.macros.fat,
                            fiber: mealResponse.macros.fiber
                        ),
                        ingredients: mealResponse.ingredients.map { ingredientResponse in
                            Ingredient(
                                id: UUID(),
                                name: ingredientResponse.name,
                                amount: ingredientResponse.amount,
                                unit: ingredientResponse.unit,
                                calories: ingredientResponse.calories,
                                macros: MacroBreakdown(protein: 0, carbs: 0, fat: 0, fiber: 0), // Simplified
                                isOptional: ingredientResponse.isOptional,
                                substitutes: ingredientResponse.substitutes
                            )
                        },
                        instructions: mealResponse.instructions,
                        prepTime: mealResponse.prepTime,
                        cookTime: mealResponse.cookTime,
                        servings: mealResponse.servings,
                        difficulty: CookingSkill(rawValue: mealResponse.difficulty) ?? .beginner,
                        tags: mealResponse.tags,
                        imageUrl: nil
                    )
                },
                shoppingList: jsonResponse.shoppingList,
            ...
            throw AIError.parsingError("Failed to parse meal plan: \(error.localizedDescription)")
        }
    }
...
// MARK: - Response Models for Parsing
...
private struct ExerciseResponse: Codable {
    let name: String
    let description: String
    ...
    let tips: [String]
    let modifications: [ModificationResponse]
}

private struct ModificationResponse: Codable {
    let type: String
    let description: String
    let instructions: String
}

private struct MealPlanResponse: Codable {
    let title: String
    let description: String
    let targetCalories: Int
    let macroBreakdown: MacroResponse
...
private struct MacroResponse: Codable {
    let protein: Int
    let carbs: Int
    let fat: Int
    let fiber: Int
...
private struct IngredientResponse: Codable {
    let name: String
    let amount: Double
    let unit: String
    let calories: Int
